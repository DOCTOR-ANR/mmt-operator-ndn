...
ReportFactory.createSecurityRealtimeReport = function (fPeriod) {
    ...
    var appendData = function( msg ){
        ...
        if( msg[4]!=103 && msg[ COL.VERDICT_COUNT.id ] > 0 )
    ...
    var cTable = MMTDrop.chartFactory.createTable({
        getData: {
            getDataFn: function (db) {
                ...
                arr = [];
                for( var i in DATA ){
                    	let arr_data=getDataToShow( DATA[i] );
			if(arr_data.length>0) arr.push( arr_data );
                }
    ...
    function addAlerts( data ) {
	    ...
	    if(row_data.length>0) row.data( row_data );
        ...
        for( var i in indexOfNewRow ){
            ...
            if(new_row.length>0) table.api().row.add( new_row );
        }
        indexOfNewRow = [];
